...
def getUniquelyMappingReads(bamFile):
    ...
    statsFile = filter(lambda x: x.count(bamName) ==
                       1 and x.count('stats.') == 1, bamFiles)
    if len(statsFile) == 1:
        ...
        statsFile = filter(lambda x: x.count(bamName) ==
                           1 and x.count('stats.concise') == 1, bamFiles)
        print('USING STATS FILE %s' % (pathFolder+'/'+statsFile[0]))
        samDict = parseSamHeader(pathFolder+'/'+statsFile[0])
        return int(samDict['UniquelyMappingSequenceTags'])
    else:
...
def mapBamToGFF(bamFile, gff, sense='both', unique=0, extension=200, floor=0, density=False, rpm=False, binSize=25, clusterGram=None, matrix=None, raw=False, includeJxnReads=False):
    ...
    if clusterGram:
        ...
        for i in range(0, len(gff), 1):
            line = gff[i]
            gffLocus = Locus(line[0], int(line[3]),
                             int(line[4]), line[6], line[1])
        ...
        newGFF.append(['GENE_ID', 'locusLine'] + [str(x*binSize)+'_' +
                      bamFile.split('/')[-1] for x in range(1, max(binSizeList)+1, 1)])
    # setting up a maxtrix table
    if matrix:
        newGFF.append(['GENE_ID', 'locusLine'] + ['bin_'+str(n)+'_' +
                      bamFile.split('/')[-1] for n in range(1, int(matrix)+1, 1)])
    ...
    for line in gff:
        ...
        reads = bam.getReadsLocus(
            searchLocus, 'both', unique, 'none', includeJxnReads)
        # now extend the reads and make a list of extended reads
        extendedReads = []
        for locus in reads:
            if locus.sense() == '+' or locus.sense() == '.':
                locus = Locus(locus.chr(), locus.start(),
                              locus.end()+extension, locus.sense(), locus.ID())
            if locus.sense() == '-':
                locus = Locus(locus.chr(), locus.start()-extension,
                              locus.end(), locus.sense(), locus.ID())
            extendedReads.append(locus)
        if gffLocus.sense() == '+' or gffLocus.sense == '.':
            senseReads = filter(lambda x: x.sense() ==
                                '+' or x.sense() == '.', extendedReads)
        ...
        if density:
            ...
            if floor > 0:
                ...
                keys = filter(lambda x: (
                    senseHash[x]+antiHash[x]) > floor, keys)
            # coordinate filtering
            keys = filter(lambda x: gffLocus.start()
                          < x < gffLocus.end(), keys)
            ...
            if clusterGram or matrix:
                ...
                if gffLocus.sense() == '+' or gffLocus.sense() == '.' or gffLocus.sense() == 'both':
                    ...
                    while n < nBins:
                        ...
                        binDen = float(
                            sum([senseHash[x]+antiHash[x] for x in binKeys]))/binSize
            ...
            # for regular old density calculation
                ...
                senseTotalDen = float(
                    sum([senseHash[x] for x in keys]))/gffLocus.len()
                antiTotalDen = float(sum([antiHash[x]
                                     for x in keys]))/gffLocus.len()
                if rpm:
                ...
                if sense == 'both' or sense == '.':
                    if gffLocus.sense() == '+' or gffLocus.sense() == '.':
                        readLine = '+' + \
                            ':%s' % (round(senseTotalDen, 4)) + ';' + \
                            '-' + ':%s' % (round(antiTotalDen, 4))
                    else:
                        readLine = '+' + \
                            ':%s' % (round(antiTotalDen, 4)) + ';' + \
                            '-' + ':%s' % (round(senseTotalDen, 4))
        ...
        # if not cluster or density simply return reads
            ...
            if sense == 'both' or sense == '.':
                if gffLocus.sense() == '+' or gffLocus.sense() == '.':
                    readLine = '+'+':' + join([str(locus.start()) for locus in senseReads], ',') + \
                        ';' + '-'+':' + join([str(locus.start())
                                             for locus in antiReads], ',')
                else:
                    readLine = '+'+':' + join([str(locus.start()) for locus in antiReads], ',') + \
                        ';'+'-'+':' + join([str(locus.start())
                                           for locus in senseReads], ',')
            elif sense == '+':
                readLine = gffLocus.sense()+':' + \
                    join([str(locus.start()) for locus in senseReads], ',')
            elif sense == '-':
                readLine = string.translate(gffLocus.sense(
                ), senseTrans)+':' + join([str(locus.start()) for locus in antiReads], ',')
            newGFF.append(line+[readLine])
...
def convertEnrichedRegionsToGFF(enrichedRegionFile):
    ...
    for line in enrichedRegions:
        line = line[:-1].split('\t')
        newLine = ['chr'+line[0], 'row_' +
                   str(i), line[4], line[1], line[2], '', '.', '', 'row_'+str(i), '']
...
def main():
    ...
    if options.input and options.bam:
        ...
        if options.cluster:
            ...
            newGFF = mapBamToGFF(bamFile, gffFile, options.sense, options.unique, int(
                options.extension), options.floor, options.density, options.rpm, options.cluster, True, None, False, options.jxn)
        elif options.matrix:
            print('mapping to GFF and making a matrix with fixed bin number')
            newGFF = mapBamToGFF(bamFile, gffFile, options.sense, options.unique, int(
                options.extension), options.floor, options.density, options.rpm, 25, None, options.matrix, False, options.jxn)
            ...
            if options.total:
                ...
                newGFF = mapBamToGFF(bamFile, gffFile, options.sense, options.unique, int(
                    options.extension), options.floor, options.density, options.rpm, 25, None, None, False, options.jxn)
            else:
                newGFF = mapBamToGFF(bamFile, gffFile, options.sense, options.unique, int(
                    options.extension), options.floor, options.density, options.rpm, 25, None, None, True)
